...
class ui_table_py(_ui_table_helper_py):
	def __init__(self):
	...
	def _set_data(self, data):
		...
		def on_blur(ev):
			...
			if not _check_type(text, field_types[c_num]): return
			row[c_num] = text
			self._event_listener_data()
		...
		def on_paste(ev):
			...
			if select_cell_0 is not None:
				...
				for i,t in enumerate(text):
					text[i] = [t1.strip() for t1 in t.split('\t') if t1.strip() != '']
				...
				for i,text2 in enumerate(text):
					for j,t in enumerate(text2):
						if row_num+i < len(tbody.children) and col_num+1+j < len(tbody.children[row_num+i].children):
							if not _check_type(t, field_types[col_num+j]):
