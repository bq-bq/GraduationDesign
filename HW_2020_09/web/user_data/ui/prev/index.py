...
class DataHub:
	...
	def dispose(self):
		...
		self.data2obj = {} # table_name -> list of [data_name, ui_component objects]
		self.var = {} # var_name -> table_data
		self.var2obj = {} # var_name -> list of [var_name, ui_component objects]
		self.var2objdata = {} # var_name -> list of [data_name, ui_component objects]
		self.change = set()
		self.event_queue = []
	...
	def __repr__(self):
		...
		repr += 'var:\n'
		for var_name in self.var: repr += '\t' + var_name + '\n'
		...
		repr += 'var2obj\n'
		print_data(self.var2obj)
		repr += 'var2objdata\n'
		print_data(self.var2objdata)
		return repr
		...
		if 'data' in obj.config:
			...
			for data_name, table_name in data.items():
				if table_name[0] == '$':
					var_name = table_name
					if remove:
						self.var2objdata[var_name].remove((data_name, obj_id))
						if len(self.var2objdata) == 0:
							del self.var2objdata[var_name]
					else:
						if var_name not in self.var2objdata:
							self.var2objdata[var_name] = set()
						# if (data_name, obj_id) not in self.var2objdata[var_name]:
						assert (data_name, obj_id) not in self.var2objdata[var_name]
						self.var2objdata[var_name].add((data_name, obj_id)) # when var[var_name] or data[var[var_name]] is changed, will call obj.set_data(data_name, data[var[var_name]])
						# update object data
						if var_name in self.var and self.var[var_name] is not None:
							var_value = self.var[var_name]
							table_name = new_var_value['rows'][0][0]
					...
					if remove:
						...
						# if (data_name, obj_id) not in self.data2obj[table_name]:
						...
						if len(self.data[table_name]) > 1:
							updates.append((obj, data_name, self.data[table_name]))
		if 'vars' in obj.config:
			variables = obj.config['vars']
			for data_name, var_name in variables.items():
				if len(var_name) == 0: continue
				assert var_name[0] == '$', 'Value "%s" of var "%s" should start with a "$".' % (var_name, data_name) 
				if remove:
					self.var2obj[var_name].remove((data_name, obj_id))
					if len(self.var2obj[var_name]) == 0:
						del self.var2obj[var_name]
				else:
					if var_name not in self.var2obj:
						self.var2obj[var_name] = set()
					if (data_name, obj_id) not in self.var2obj[var_name]:
						self.var2obj[var_name].add((data_name, obj_id)) # when var[var_name] is changed, will call obj.set_data(var_name, var[var_name])
					# update object data
					if var_name in self.var and self.var[var_name] is not None:
						updates.append((obj, var_name, self.var[var_name]))
		if remove:
	...
	def _remove_headless_table_data(self):
		in_use = set(self.data2obj.keys())
		for var_name in self.var2objdata:
			if var_name in self.var and self.var[var_name] is not None:
				table_names.add(self.var[var_name]['rows'][0][0])
		not_in_use = self.data.keys() - in_use
		for table_name in not_in_use:
			del self.data[table_name]
		in_use = self.var2obj.keys() | self.var2objdata.keys()
		not_in_use = self.var.keys() - in_use
		for var_name in not_in_use:
	...
	def _update_data(self, obj0, table_name):
		...
		if table_name in self.data2obj:
			for data_name, obj_id in self.data2obj[table_name]:
				...
				updates.append((obj, data_name, self.data[table_name]))
		for var_name, table_name2 in self.var.items():
			if table_name2 is None or 'rows' not in table_name2: continue
			if len(table_name2['rows']) == 0 or len(table_name2['rows'][0]) == 0: continue
			table_name2 = table_name2['rows'][0][0]
			if table_name2 != table_name: continue
			if var_name not in self.var2objdata: continue
			for data_name, obj_id in self.var2objdata[var_name]:
				if obj_id == obj_id0: continue
				obj = self.components[obj_id]
				# obj.set_data(data_name, self.data[table_name])
				updates.append((obj, data_name, self.data[table_name]))
	...
	def _update_var(self, obj, var_name, new_var_value):
		obj_id = id(obj) if obj is not None else None
		self.var[var_name] = new_var_value
		to_sync = False
		updates = []
		if var_name in self.var2obj:
			for data_name, obj_id2 in self.var2obj[var_name]:
				if obj_id2 == obj_id: continue
				obj2 = self.components[obj_id2]
				# obj2.set_data(data_name, new_var_value)
				updates.append((obj2, data_name, new_var_value))
		if var_name in self.var2objdata:
			if new_var_value is None:
				...
				if table_name not in self.data:
					self.data[table_name] = {'__update_time__':-1}
					to_sync = True # need to fetch a table whose name is given in new_var_value['rows'][0][0]
				table_data = self.data[table_name]
			for data_name, obj_id2 in self.var2objdata[var_name]:
				obj2 = self.components[obj_id2]
				# obj2.set_data(data_name, table_data)
				updates.append((obj2, data_name, table_data))
		return to_sync, updates
	...
	def _handle_change_event(self, obj, data_name):
		...
		if data_name in data:
			...
			if table_name[0] == '$':
				assert table_name in self.var, '变量"%s"未被使用' % table_name
				table_name_data = self.var[table_name]
			...
			updates1 = self._update_data(obj, table_name)
			updates.extend(updates1)
		vars = obj.config['vars'] if 'vars' in obj.config else {}
		if data_name in vars:
			has_data = True
			var_name = vars[data_name]
			new_var_value = obj.get_data(data_name)
			to_sync, updates2 = self._update_var(obj, var_name, new_var_value)
			updates.extend(updates2)
		# print('Warning: no such data or var "%s" in object "%s"' % (data_name, obj_type))
		if len(updates) > 0:
	...
	def handle_ui_event(self, obj, ev, ev_arg):
		...
		if ev == 'change': # a special event which notify the DataHub that a data/var with name ev_arg is changed
			...
			for var_name in self.var:
				var_table = self.var[var_name]
				if var_table is not None and 'rows' in var_table and var_table['rows'][0][0] == ev_arg:
					self._update_var(None, var_name, None)			
			...
			if 'error' in data:
				...
				for table_name, tb_data in data.items():
					if len(tb_data) == 1:
						...
						self.data[table_name] = tb_data
						updates = self._update_data(None, table_name)
						for obj, name, data1 in updates:
							obj.set_data(name, data1)
			next_ev()
...
def make_ui(ui_json, page, data_hub):
...
def make_component(ui_json, is_editing, data_hub=None):
	try:
		...
		config = {'__meta__': {'name': '错误', 'tag': 'div', 'text': '错误', 'init': '', 'files': [], 'data': [], 'vars': [], 'events': [], 'class': {}, 'attr': {}, 'style': {}}, 'tag': 'div', 'text': '错误', 'files': [], 'data': {}, 'events': {}, 'vars': {}, 'class': 'alert alert-error', 'style': {}, 'attr': {'role': 'alert'}}
		return Custom_Component(config, is_editing)
...
def copy_config(config, is_editing):
	...
	if 'vars' in config: c['vars'] = dict(config['vars']).copy()
	...
	def set_data(self, data_name, data):
		def remove_prev_var():
			...
			for var in to_remove:
				del self.config['vars'][var]
		if data_name == 'data':
			self.data = data
			self.child_data = {}
			children = []
			child_data_prefix = None
			if 'child_data' in self.config['vars']:
				child_data_prefix = self.config['vars']['child_data']
			if child_data_prefix is not None and len(self.ui_json['children']) > 0:
				...
				def change_var(ui_json, prefix, name):
					...
					if 'config' in ui_json:
						for var_name, var_data_name in ui_json['config']['vars'].items():
							if var_data_name == prefix: to_change.append(var_name)
					for var_name in to_change:
						ui_json['config']['vars'][var_name] = name
...
def get_source_ui_json(config_template):
	...
	if 'text' in config_template and config_template['text'].strip() != '': config['text'] = config_template['text']
...
class UI_Editor(Custom_Component):
	...
	def get_data(self, data_name):
		if data_name == 'config':
			...
			raise AssertionError('Unknown var: ' + data_name)
	def set_data(self, data_name, data):
		if data_name == 'config':
